...
ggplot(lineouts_lost_by_opposition, aes(opposition, team)) + geom_tile(aes(fill = lineouts_lost))
```
...
ggplot(lineouts_won_by_opposition, aes(opposition, team)) + geom_tile(aes(fill = lineouts_won))
```

```{r, fig.width=7, fig.height=4, echo = FALSE}
lineouts_win_percentage_by_opposition <- tournament_data %>% 
  group_by(team, opposition) %>%
  summarise(
    total_lineouts = sum(total_lineouts),
    lineouts_won = sum(lineouts_won), 
...
ggplot(lineouts_win_percentage_by_opposition, aes(opposition, team)) + geom_tile(aes(fill = win_percentage))
```
